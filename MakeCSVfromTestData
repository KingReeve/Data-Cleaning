...
for i in dataFromFile:
    ...
    templist = i.partition("Cmd: ") # couldn't find elegant solution, gave up and made a temporary, size-3 tuple and used re.search because it will only find the first occurrence.  The data comes after actual, and the important data is after the Cmd value.
    y = re.search(r'(?<=Actual: )(.*?)(?= )', templist[2]).group() #this will be the values
    dataDict[x].append(y)

if not dataDict:
    raise SystemExit("File contained no appropriate data.") #dictionaries are false if empty.
